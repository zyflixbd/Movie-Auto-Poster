...
def generate_caption(movie: dict) -> str:
    ...
    WEBSITE_LINE = "🟤মুভিটি দেখতে 【 গুগলে সার্চ করুন > 𝐌𝐲𝐜𝐢𝐧𝐞𝐛𝐝.𝐜𝐨𝐦 লিখে】"

    prompt = f"""তুমি একটি বাংলা মুভি রিভিউ পেজের সেরা কন্টেন্ট রাইটার। তোমার লেখা পড়লে মানুষ মুভিটা দেখতে আগ্রহী হয়ে যায়।
নিচের হলিউড মুভিটির জন্য একটি প্রাণবন্ত, engaging বাংলা Facebook পোস্ট লেখো যা পড়া শুরু করলে শেষ না করে থামা যায় না।
...
পোস্টের কাঠামো (এই ক্রমে লিখবে):

১. শুরুতে একটি চমকপ্রদ হেডলাইন — এমন যা দেখলেই মানুষ থামবে (ইমোজি দিয়ে)
২. কাহিনীর এমন একটা দিক তুলে ধরো যা curiosity তৈরি করে — spoiler দেবে না
৩. অভিনয়, পরিচালনা, ভিজ্যুয়াল নিয়ে উত্তেজনাপূর্ণ ভাষায় লেখো
৪. রেটিং ও কাস্ট উল্লেখ করো
৫. একটা strong call-to-action দিয়ে শেষ করো

পোস্টের একদম শেষে, হ্যাশট্যাগের ঠিক আগে, নিচের লাইনটি হুবহু বসাবে:
{WEBSITE_LINE}

হ্যাশট্যাগ (সর্বোচ্চ ৩-৪টি মাত্র):
- প্রথম হ্যাশট্যাগ অবশ্যই: #{title.replace(' ', '')}_{release}
- বাকি ২-৩টি genre/mood অনুযায়ী বাংলা বা ইংরেজি

কঠোর নিষেধ:
❌ "অবশ্যই", "নিশ্চিতভাবে" দিয়ে শুরু করবে না
❌ website লাইনটি একটুও পরিবর্তন করবে না
❌ ৩০০ শব্দের বেশি লিখবে না
❌ ৩-৪টির বেশি হ্যাশট্যাগ দেবে না"""
